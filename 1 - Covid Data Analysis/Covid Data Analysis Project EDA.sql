...
--Counting rows to compare against the other tables to ensure that all data was transferred.
...
WITH max_vaccines_per_location AS (
	SELECT
		continent,
		location,
		MAX(total_vaccinations) AS total_vaccinations_per_location
	FROM covid_vaccines
	WHERE continent IS NOT NULL
	GROUP BY 
		continent,
...
total_vaccinations_per_continent AS (
	SELECT
		continent,
		SUM(total_vaccinations_per_location) AS total_vaccinations_per_continent
	FROM max_vaccines_per_location
	GROUP BY continent
	ORDER BY total_vaccinations_per_continent DESC
)

SELECT 
	continent,
	total_vaccinations_per_continent,
	ROUND(total_vaccinations_per_continent / (
		SELECT 
			SUM(total_vaccinations_per_continent)
		FROM total_vaccinations_per_continent) * 100, 2) || '%' AS percentage_of_total_global_vaccinations
FROM total_vaccinations_per_continent


--Ranking the countries based on the amount of new tests performed each day since the start of the pandemic. 
--Aggregating the test amounts for a better high-level view. Elinating obvious outliers. 
